...
export default function SearchBar({ placeholder, value, onChange, collections, onCollectionClick }: SearchBarProps) {
  const [focused, setFocused] = useState(false);
  const wrapperRef = useRef<HTMLDivElement>(null);

  const q = value.toLowerCase().trim();
  const matched = q && collections
    ? collections.filter(
        (c) => c.name.toLowerCase().includes(q)
      ).slice(0, 6)
    : [];

  const showDropdown = focused && matched.length > 0;
